...
async function batchOne() {
    await describe({ name: 'FirstBatch', id: 'first' }, async () => {
        await Promise.all([
            it({ name: 'Test 1.1', parentId: 'first' }, async () => {
                await simulateDelay(100);
                assertEqual(true, true, 'Test 1.1 passed');
            }),
            it({ name: 'Test 1.2', parentId: 'first' }, async () => {
                await simulateDelay(75);
                assertEqual(true, true, 'Test 1.2 passed');
            }),
            it({ name: 'Test 1.3', parentId: 'first' }, async () => {
...
async function batchTwo() {
    await describe({ name: 'SecondBatch', id: 'second' }, async () => {
        await Promise.all([
            it({ name: 'Test 2.1', parentId: 'second' }, async () => {
                await simulateDelay(125);
                assertEqual(true, true, 'Test 2.1 passed');
            }),
            it({ name: 'Test 2.2', parentId: 'second' }, async () => {
                await simulateDelay(150);
                assertEqual(1, 2, 'This test should fail');
            }),
            it({ name: 'Test 2.3', parentId: 'second' }, async () => {
...
async function batchThree() {
    await describe({ name: 'thirdBatch', id: 'third' }, async () => {
        await Promise.all([
            it({ name: 'Test 3.1', parentId: 'third' }, async () => {
                await simulateDelay(200);
                assertEqual(true, true, 'Test 3.1 passed');
            }),
            it({ name: 'Test 3.2', parentId: 'third' }, async () => {
                await simulateDelay(225);
                assertEqual(true, true, 'Test 3.2 passed');
            }),
            it({ name: 'Test 3.3', parentId: 'third' }, async () => {
